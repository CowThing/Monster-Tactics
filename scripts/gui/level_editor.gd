...
var paint_tool = preload("res://scripts/gui/level_editor_paint_tools.gd").new(self)
...
onready var tool_buttons = get_node("ToolPanel/ToolButtons")
...
func paint_unit(pos, team):
	...
	if current_map.get_tile(pos) in [1, 2]:
		# Can't place on building or water
		return
		...
		if ev.pressed:
			...
			if current_paint_mode == PAINT_TILES:
				if ev.button_index == BUTTON_LEFT:
					paint_tool.selected_tool.start(target_pos, selected_tile)
				else:
					paint_tool.eraser.start(target_pos, 2)
		...
		if new_target_pos != target_pos:
			...
			if current_paint_mode == PAINT_TILES:
				if Input.is_mouse_button_pressed(BUTTON_LEFT):
					paint_tool.selected_tool.update(target_pos)
				elif Input.is_mouse_button_pressed(BUTTON_RIGHT):
					paint_tool.eraser.update(target_pos)
...
func _on_TabContainer_tab_selected( tab ):
	...
	tool_buttons.set_hidden(tab != PAINT_TILES)
...
func _on_ToolButtons_button_selected( button_idx ):
	paint_tool.set_selected_tool(button_idx)
